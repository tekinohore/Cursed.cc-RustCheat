...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(15, 15));
    ...
    if (*v) {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_CheckboxActive, it_filled->second), 4);
        RenderCheckMark(ImVec2(check_bb.Min.x + 2, check_bb.Min.y - 1), GetColorU32(ImGuiCol_CheckMark, it_filled->second), 12);
    }
    else {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_Checkbox), 4);
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_CheckboxBorder), 4);
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_CheckboxActive, it_hover->second), 4);
    }

    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Min.x + 21, check_bb.Min.y + 2), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    IM_ASSERT((flags & (ImGuiComboFlags_NoPreview)) != (ImGuiComboFlags_NoPreview)); // Can't use both flags together
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 16), window->DC.CursorPos + ImVec2(expected_w, 36));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 16));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_Combo), 3, popup_open ? 3 : 15);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_CheckboxBorder), 3);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        RenderArrow(window->DrawList, ImVec2(frame_bb.Max.x - 15, frame_bb.Min.y + 3), GetColorU32(ImGuiCol_Text), 10);
    }

    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(ImVec2(frame_bb.Min.x + 5, frame_bb.Min.y + 20 / 2 - label_size.y / 2), value_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));

    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y - 18), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true); 
    const float w = CalcItemWidth();
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 25), window->DC.CursorPos + ImVec2(w, 33));
    const ImRect total_bb(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y), ImVec2(frame_bb.Max.x + 2, frame_bb.Max.y + 25));
    ...
    // Draw frame
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 4), GetColorU32(ImGuiCol_Slider), 4);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(grab_bb.Min.x + 2, frame_bb.Min.y + 4), GetColorU32(ImGuiCol_SliderActive), 4);
    ...
    // Slider grab
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 5, frame_bb.Min.y + 4), 5, GetColorU32(ImGuiCol_SliderGrab), 360);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 24), label);
    ...
    window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - 10, frame_bb.Min.y - 20), ImVec2(grab_bb.Max.x + 10, frame_bb.Min.y - 5), ImColor(42, 47, 55, int(it_hover->second * 255)), 3);
    PushStyleColor(ImGuiCol_Text, ImVec4(97 / 255.f, 105 / 255.f, 114 / 255.f, it_hover->second));
    RenderTextClipped(ImVec2(grab_bb.Min.x - 10, frame_bb.Min.y - 20), ImVec2(grab_bb.Max.x + 10, frame_bb.Min.y - 5), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    PopStyleColor();
