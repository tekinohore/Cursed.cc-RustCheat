...
void ImGui::RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, float sz)
{
    float thickness = ImMax(sz / 12.f, 1.0f);
    sz -= thickness * 0.1f;
...
void ImGui::RenderCheckMark(ImVec2 pos, ImU32 col, float sz)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    float thickness = ImMax(sz / 6.f, 1.0f);
    sz -= thickness * 0.1f;
    pos += ImVec2(thickness * 0.5f, thickness * 0.5f);
    ...
    window->DrawList->PathLineTo(ImVec2(bx + third * 2.0f, by - third * 1.5f));
